...
// TestCards runs a series of tests to exercise Card behavior from the
...
func TestCards(t *testing.T) {
	...
	t.Run("CardCRUD", tests.CardCRUD)
}

// CardTests holds methods for each card subtest. This type allows
...
func (c *CardTests) List(t *testing.T) {
...
func (c *CardTests) CreateRequiresFields(t *testing.T) {
...
func (c *CardTests) CardCRUD(t *testing.T) {
	...
	{ // CREATE
		...
		if created["date_created"] == "" || created["date_created"] == nil {
			t.Fatal("expected non-empty card date_created")
		}
		if created["date_updated"] == "" || created["date_updated"] == nil {
			t.Fatal("expected non-empty card date_updated")
		}
	...
	{ // READ
		url := fmt.Sprintf("/v1/cards/%s", created["id"])
		...
		// Fetched card should match the one we created.
		if diff := cmp.Diff(created, fetched); diff != "" {
			t.Fatalf("Retrieved card should match created. Diff:\n%s", diff)
		}
	}
	...
	{ // UPDATE
		...
		// Updated card should match the one we created.
